# VARVEL Engagement Report — alsco

**Scope:** 188.34.187.32/32  
**Authorized by:** marcus · operator · sess-1016
...
- Host: `sandbox-royal.securegateway.com` · confidence: **CONFIRMED** (85%)
...
- Exploit: **wildcard cert -> vhost confusion -> unauthorized admin access** — _proposed_

### [MEDIUM] TLS overly broad wildcard certificate  `F-01`
- Host: `sandbox.securegateway.com` · confidence: **SUSPECTED** (69%)
- Validation: **CLAIMED-UNVALIDATED** — never reproduced by the validator gate
...
- **HELD** `Read` — enclave-denied (alsco)
- **HELD** `Bash` — enclave-denied (alsco)
- **HELD** `Read` — enclave-denied (alsco)
...
### [5/5] Active exploitation attempt ×35
...
### [4/5] Web content discovery (path brute-force) ×3
...
### [3/5] Vulnerability checks (known-exposure probes) ×3
...
### [2/5] Service / version detection (banner grab) ×23
...
### [2/5] HTTP method / CORS / header probe ×3
...
### [2/5] Web crawl (following the site’s own links) ×3
...
Operational-stealth profile: **NORMAL**. Peak loudness 5/5 stayed ≤ ceiling 5, but cumulative noise reached 261/250 noise pts — 11 OVER the normal budget (1 HITL-authorized).

- Noise budget: **261/250** noise-points (**104%** of cap, state: **EXHAUSTED**)
- Cumulative noise budget **exceeded** by **11** noise-point(s)
