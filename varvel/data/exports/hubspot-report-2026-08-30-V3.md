...
- Host: `hubspot` · confidence: **SUSPECTED** (69%)
- Validation: **CLAIMED-UNVALIDATED** — never reproduced by the validator gate
...
- Tool-lane budget (jsminer / OOB / DOM-XSS canary / authz seeding — a bucket SEPARATE from the agent step and noise budgets): **9/200** governed requests, wall-clock cap **600s** — not exhausted.

## OPSEC — detection footprint

Overall detection risk: **HIGH** — peak loudness **5/5**, weighted **3.9/5**, across **205** recorded action(s).

What a defender would have observed (loudest first):

### [5/5] Active exploitation attempt ×13
...
### [4/5] Artifact written to a target host ×2
...
### [2/5] Service / version detection (banner grab) ×29
...
### [1/5] HTTP fingerprint (headers + favicon) ×9
...
Operational-stealth profile: **NORMAL**. Peak loudness 5/5 stayed ≤ ceiling 5, but cumulative noise reached 540/300 noise pts — 240 OVER the normal budget (3 HITL-authorized).

- Noise budget: **540/300** noise-points (**180%** of cap, state: **EXHAUSTED**)
- Cumulative noise budget **exceeded** by **240** noise-point(s)
- Peak-loudness ceiling honored: emitted **5/5** ≤ ceiling **5/5**
- HITL-authorized over-ceiling actions: **3** (noise was raised only with explicit approval)
...
- `varvel-workspace:opsec-ledger.md` (file) — **PENDING** cleanup · `del opsec-ledger.md`
- `varvel-workspace:post-ex-summary.json` (file) — **PENDING** cleanup · `del post-ex-summary.json`
...
- Phases audited: **4** · productive: **2** · no-progress: **0** · longest stall: **2**
