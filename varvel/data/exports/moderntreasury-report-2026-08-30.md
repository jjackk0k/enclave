# VARVEL Engagement Report — moderntreasury

**Scope:** 104.249.160.4/32, 104.249.161.4/32, 3.162.38.38/32, 3.162.38.72/32, 3.162.38.86/32, 3.162.38.97/32  
**Authorized by:** marcus · operator · sess-1012

## Executive summary

- Hosts in scope: **2**
- Findings: **2**  (confirmed: **2**, critical: **0**)
- Validation (objective oracle): **0 validated · 2 claimed-unvalidated · 0 refuted · 0 untestable**
- Risk levels: **0 high · 2 medium · 0 info**
- Exploits proven: **0**
- Governance holds (actions the platform blocked in-flight): **5**

## Findings

### [MEDIUM] CORS reflects an arbitrary Origin WITH credentials — cross-origin data theft possible  `VC-1`
- Host: `cdn.moderntreasury.com` · confidence: **CONFIRMED** (85%)
...
- Exploit: **CORS reflect-with-credentials -> cross-origin data theft** — _proposed_

### [MEDIUM] CORS reflects an arbitrary Origin WITH credentials — cross-origin data theft possible  `F-01`
- Host: `app.moderntreasury.com` · confidence: **CONFIRMED** (85%)
- Validation: **CLAIMED-UNVALIDATED** — never reproduced by the validator gate
- Maps to: A07:2021 Identification & Authentication Failures · MITRE TA0006 Credential Access

## Governance & scope

- **HELD** `Read` — enclave-denied (moderntreasury)
- **HELD** `Bash` — enclave-denied (moderntreasury)
- **HELD** `Bash` — enclave-denied (moderntreasury)
- **HELD** `Bash` — enclave-denied (moderntreasury)
- **HELD** `Read` — enclave-denied (moderntreasury)
...
Overall detection risk: **HIGH** — peak loudness **5/5**, weighted **4.2/5**, across **83** recorded action(s).
...
### [2/5] Service / version detection (banner grab) ×32
...
Operational-stealth profile: **NORMAL**. Stayed under the normal budget: 271/300 noise pts (90%), peak loudness 5/5 ≤ ceiling 5, 1 HITL-authorized override(s).

- Noise budget: **271/300** noise-points (**90%** of cap, state: **CRITICAL**)
...
Current detection risk **HIGH** (4.2/5); applying the moves below projects **MODERATE** (2.4/5, −1.8).
...
- **[MED]** CORS reflects an arbitrary Origin WITH credentials — cross-origin data theft possible (VC-1): Rotate exposed credentials; enforce strong secrets + MFA; remove hardcoded keys.
- **[MED]** CORS reflects an arbitrary Origin WITH credentials — cross-origin data theft possible (F-01): Rotate exposed credentials; enforce strong secrets + MFA; remove hardcoded keys.

## Autonomy integrity

- Phases audited: **4** · productive: **3** · no-progress: **1** · longest stall: **1**
