# VARVEL Engagement Report — sheer

**Scope:** 185.120.70.196/32, 185.120.70.212/32  
**Authorized by:** marcus · operator · sess-1015

## Executive summary

- Hosts in scope: **2**
- Findings: **4**  (confirmed: **2**, critical: **0**)
- Validation (objective oracle): **0 validated · 4 claimed-unvalidated · 0 refuted · 0 untestable**
- Risk levels: **0 high · 4 medium · 0 info**
- Exploits proven: **0**
- Governance holds (actions the platform blocked in-flight): **7**

## Findings

### [MEDIUM] TLS: overly broad wildcard certificate  `TLS-WILDCARD-BROAD`
- Host: `www.sheer.com` · confidence: **CONFIRMED** (85%)
...
- Exploit: **wildcard subdomain abuse chain** — _proposed_

### [MEDIUM] TLS: overly broad wildcard certificate  `TLS-WILDCARD-BROAD`
- Host: `my.sheer.com` · confidence: **CONFIRMED** (85%)
- Validation: **CLAIMED-UNVALIDATED** — never reproduced by the validator gate
- Maps to: A02:2021 Cryptographic Failures · MITRE TA0009 Collection

### [MEDIUM] TLS: overly broad wildcard certificate  `F-01`
- Host: `www.sheer.com` · confidence: **SUSPECTED** (69%)
- Validation: **CLAIMED-UNVALIDATED** — never reproduced by the validator gate
- Note: _validator gate: no objective oracle cited_
- Maps to: A02:2021 Cryptographic Failures · MITRE TA0009 Collection

### [MEDIUM] TLS: overly broad wildcard certificate  `F-02`
- Host: `my.sheer.com` · confidence: **SUSPECTED** (69%)
- Validation: **CLAIMED-UNVALIDATED** — never reproduced by the validator gate
- Note: _validator gate: no objective oracle cited_
- Maps to: A02:2021 Cryptographic Failures · MITRE TA0009 Collection
...
- **HELD** `Read` — enclave-denied (sheer)
- **HELD** `Read` — enclave-denied (sheer)
- **HELD** `Bash` — enclave-denied (sheer)
- **HELD** `Read` — enclave-denied (sheer)
- **HELD** `Bash` — enclave-denied (sheer)
- **HELD** `Bash` — enclave-denied (sheer)
- **HELD** `Bash` — enclave-denied (sheer)
...
Overall detection risk: **HIGH** — peak loudness **5/5**, weighted **4.3/5**, across **64** recorded action(s).

What a defender would have observed (loudest first):

### [5/5] Active exploitation attempt ×28
...
### [2/5] Service / version detection (banner grab) ×16
...
Operational-stealth profile: **NORMAL**. Stayed under the normal budget: 224/300 noise pts (75%), peak loudness 5/5 ≤ ceiling 5, 1 HITL-authorized override(s).

- Noise budget: **224/300** noise-points (**75%** of cap, state: **WARN**)
